...
** What is Leitner?
=leitner.el= is an Emacs package that applies the classic =Leitner box system= to your note-taking workflow, this is for /re-reading/ your notes and /revising/ them, not for active recall(though it can be used that way)

It is designed for people who take extensive, *long-form* notes but rarely revisit them, or find themselves overwhelmed by the volume of these long notes to review. Rather than letting them idle away in a folder somewhere, you simply use =leitner-add-file= which takes care of when to /resurface/ the file for your next re-reading session.
...
** How It Works
Each box has a defined review interval (Box 1 = 1 day, Box 2 = 3 days). Files are only shown for review when their interval has elapsed. Once a file reaches the final box and is rated "Good," it /graduates/ and is removed from the review queue.
...
- /Bad/: Resets the file straight back to Box 1 (forgot important details, note felt unfamiliar to you, or struggled to recall).

*There are a couple new options designed for the nuances around reading:*
- /Partial/ (new): For files too long to finish reading in one sitting. The box & ease factor are untouched. So instead of waiting out the full interval, the file is marked *paused* and comes due again the next day so you can pick up where you left off. The =dashboard= and =group-detail= view both show a =Paused= indicator until you give them a real rating. Works well with =save-place-mode=.

- /Revised/ (new): For when you *edited* or *added content* instead of being tested, the box & ease factor are also untouched, but unlike =Partial= the review counts as *complete*, so the file returns on its *normal box interval* rather than tomorrow.

- /Skip/: Leaves the file in its current box without altering its schedule.

*Good*: on a Box 4 file -> moves to Box 5
*Bad*: drops to Box 1
*Partial/paused*: stays in Box 4, but is due tomorrow instead
*Revised*: stays in Box 4, but repeats Box 4's normal interval, same wait as before
...
** Why Leitner and not something better like FSRS?
...
However, the =SM-2= hybrid mode exists for this middle ground if you really want it. When you have a deck that's large enough, you can toggle on sm-2, and it will adapt to your existing scheduling data from where you left off.
...
(setq leitner-sm2-reset-penalty 0.20) ; max EF loss on Bad, scaled down the closer the item was to Box 1
#+END_SRC
...
- /v0.3.3:/ The ease-factor on a Bad rating now scales automatically with how far the item *fell* (failing at Box 1 costs nothing, but failing at a later box costs more), so =SM-2= hybrid still tells /easy slips/ apart from *real forgetting*.
...
- /v0.3.1:/ Graduated-file browser (=leitner-review-graduated=, =G= on the dashboard) to bring individual files back to Box 1 if they become important again.
